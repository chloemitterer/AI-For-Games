...
class Agent(object):
    ...
    def emergent(self, delta):
        SteeringForce = Vector2D()
        GroupRadius = 200
        LocalGroup = []

        for agent in self.world.agents:
            if self.pos.distance(agent.pos) < GroupRadius:
                LocalGroup.append(agent)

        SteeringForce += self.wander(delta) * BEHAVIOUR_WEIGHTS['wander']/100
        SteeringForce += self.cohesion(LocalGroup) * BEHAVIOUR_WEIGHTS['cohesion']
        SteeringForce += self.separation(LocalGroup) * BEHAVIOUR_WEIGHTS['separation']
        SteeringForce += self.alignment(LocalGroup) * BEHAVIOUR_WEIGHTS['alignment']
