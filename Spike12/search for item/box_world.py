...
#            # DOWN RIGHT (i - nx + 1)
    ...
    def set_target(self, idx):
        ...
        if self.boxes[idx].marker == 'T':
            self.target.remove(self.boxes[idx])
            self.boxes[idx].marker = ''
        else:
            self.target.append(self.boxes[idx])
    ...
    def plan_path(self, search, limit):
        ...
        for target in self.target:
            target_indexes.append(target.idx)
        cls = SEARCHES[search]
        self.path = cls(self.graph, self.start.idx, target_indexes, limit)
